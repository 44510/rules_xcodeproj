...
def _profile_info_from_dict(
    ...
    return _make_profile(
        args = [_arg_env_info_from_dict(arg) for arg in profile["args"]],
        build_targets = build_targets,
        env = {
...
def _run_info_from_dict(run, *, default_xcode_configuration, top_level_deps):
    if not run:
        return _make_run()
    ...
    return _make_run(
        args = [_arg_env_info_from_dict(arg) for arg in run["args"]],
        build_targets = build_targets,
...
def _test_info_from_dict(test, *, default_xcode_configuration, top_level_deps):
    if not test:
        return _make_test()
    ...
    return _make_test(
        args = [_arg_env_info_from_dict(arg) for arg in test["args"]],
        build_targets = build_targets,
