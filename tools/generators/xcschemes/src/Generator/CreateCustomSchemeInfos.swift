...
private extension ArraySlice where Element == String {
    ...
    mutating func consumeArg(
        ...
        commandLineArguments: inout [CommandLineArgument],
        environmentVariables: inout [EnvironmentVariable],
        extensionHostIDs: [TargetID: [TargetID]],
        ...
        file: StaticString = #filePath,
        line: UInt = #line
    ) throws -> SchemeInfo.LaunchTarget? {
...
""")
        ...
        if commandLineArguments.isEmpty,
            let targetCommandLineArguments = targetCommandLineArguments[id]
        {
            commandLineArguments = targetCommandLineArguments
        }
        if environmentVariables.isEmpty,
            let targetEnvironmentVariables = targetEnvironmentVariables[id]
        {
            environmentVariables = targetEnvironmentVariables
        }

        return SchemeInfo.LaunchTarget(
            primary: target,
    ...
    mutating func consumeArg(
        ...
        let launchTarget = try consumeArg(
    ...
    mutating func consumeArg(
        ...
        var commandLineArguments = try consumeArgs(
            CommandLineArgument.self,
            in: url
        )
        var environmentVariables = try consumeArgs(
        ...
        if let id = testTargets.first?.target.key.sortedIds.first! {
            ...
            // use them
            if commandLineArguments.isEmpty,
               let aCommandLineArguments = targetCommandLineArguments[id]
            {
            ...
            // use them
            if environmentVariables.isEmpty,
               let aEnvironmentVariables = targetEnvironmentVariables[id]
            {
                var allEnvironmentVariablesTheSame = true
                for testTarget in testTargets {
                    let id = testTarget.target.key.sortedIds.first!
                    guard aEnvironmentVariables == targetEnvironmentVariables[id]
                    else {
